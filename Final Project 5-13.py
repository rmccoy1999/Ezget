...
if __name__ == '__main__': # program starts here
  ...
  while((not website) or (String_checker(website) is not True) or (HTML_checker(website) is not True)): # first checks if input is valid by string, html and port. Reason is because urllib is picky on string format.
  ...
  while((not onions_have_layers) or (not int(onions_have_layers.isdigit())) or (int(onions_have_layers) > 1000)): # after string format is checked, next checks if inputed string is a valid number. If not a number, urllib throws a hissy fit.
